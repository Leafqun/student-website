...
<script>
    export default {
        ...
        methods:{
            fetchData() {
                let _self = this;
                this.axios.get(this.url + "Tables/getTables.do", {params: {pageNum: 1}}).then(response => {
                    _self.tablesList = response.data.tablesList;
                }).catch(error => {
                    console.log(error);
                });
                this.axios.get(this.url + "Tables/getTablesNum.do").then(response => {
                    _self.pageNum = response.data.tablesNum;
                }).catch(error => {
                    console.log(error);
                });
            },
            ...
            handleCurrentChange(val) {
                ...
                let _self = this;
                this.axios.get(this.url + "Tables/getTables.do", {params:{pageNum: val}}).then( response => {
                    _self.tablesList = response.data.tablesList;
                }).catch(error => {
                    console.log(error);
                });
