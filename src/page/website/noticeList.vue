...
<script>
    ...
    export default {
        ...
        methods: {
            fetchData() {
                let _self = this;
                this.axios.get(this.url + "notice/getNotice.do", {params: {pageNum: 1}}).then(response => {
                    _self.noticeList = response.data.noticeList;
                }).catch(error => {
                    console.log(error);
                });
                this.axios.get(this.url + "notice/getNoticeNum.do").then(response => {
                    _self.pageNum = response.data.num;
                }).catch(error => {
                    console.log(error);
                });
            },
            handleCurrentChange(val) {
                ...
                let _self = this;
                this.axios.get(this.url + "notice/getNotice.do", {params:{pageNum: val}}).then( response => {
                    _self.noticeList = response.data.noticeList;
                }).catch(error => {
                    console.log(error);
                });
